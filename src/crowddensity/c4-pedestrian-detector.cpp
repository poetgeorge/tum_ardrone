...
DetecResult::DetecResult(CrowdLocation cl, int num, cv::Mat rv, double mw){
    crowdNum = num;
    crowdCentre = cl;
    resultView = rv;
    manWidth = mw;
...
//计算人群中心点，当前采用简单的求坐标均值的方法
//crowd 人体框
//imageCentre 图像中心坐标，在opencv坐标系下
//返回值：以图像中心为原点的坐标系下，人群中心点的相对坐标。
CrowdLocation getCrowdCentre(std::vector<CRect> crowd, CrowdLocation imageCentre){
    CrowdLocation crowdCentre;
    int numc = crowd.size();
    int i = 0;
    double tmpx = 0.0;
    double tmpy = 0.0;
...
DetecResult mydetector(cv::Mat myimg){
    ...
    if( src.empty() ) {
        ...
        //break;
        CrowdLocation nullCentre = CrowdLocation();
        DetecResult nullResult(nullCentre, 0, src, 0.0);
        return nullResult;
        //return src;
    }  
    CrowdLocation imageCentre;
    imageCentre.xloc = 0.5 * src.size().width;
    imageCentre.yloc = 0.5 * src.size().height;
    if (fx < 1)
    {
